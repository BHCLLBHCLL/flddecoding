...
| 序号 | 大小 | 内容 |
|------|------|------|
| block0 | 16384（ex4）或 **2560**（ex3：10×256B 槽） | ASCII 体区域名；ex4 为 `PARTS1 … PARTS32 …`（64 名）；ex3 为 10 个固定槽，名自模板 |
| sep | 16 | `12, 4, count_val, 1`（ex3 为 10，ex4 为 64） |
| block1 | `count_val × 4` | 体元数据：ex4 为 64×i32 重复模式；**ex3 为 5 桶 `[main,p2,p3,p5,p6]` 重复两遍** |
| sep | 16 | `12, 4, n_cells×2, 1`（第三字段为 `n_cells×2`，非字节数） |
| block2 | `n_cells × 8` | **每单元 8 字节** 体标志数组 |
| tail | 20 | 节尾 |

**ex3 block1 语义**：`main = n_cells − (part2 + part3 + part5 + part6)`；part4（Air_Conditioner）计入 main，不单独占 tail 桶。

### 6.3 block2 语义（体标志 / VolFlag）

- 物理上为 **每单元 8 字节**，宜按 **两个大端 i32** 解读：`(lo, hi)` 成对出现。
- 官方 ex4：前段为 `(3,4), (5,6), …` 递增对；后段因 MPI 分区与单元重排更复杂。
- **边界约束**：须 **`lo ≤ n_cells − 1`** 且 **`hi ≤ n_cells`**（官方 `lo_max = n_cells − 1`）。违反会在 `CreateVolFlag64_2` 崩溃。
- **不可全零**：scPOST `CreateVolFlag64` 需要有效索引。

**本仓库写出策略**（`fld_writer._build_volume_block2` / `_generate_vol_flag_buckets`）：

| 条件 | block2 |
|------|--------|
| `n_cells` 与模板相同 | 整块复制模板 block2 |
| ex3（模板 stem 匹配且 `count_val=10`） | 分 5 桶：bucket0 比例缩放模板；bucket1–2 线性链接；bucket3 模板+offset；bucket4 与模板相同（count 一致时） |
| 其他（如 ex2 + ex4 模板） | `_scale_entire_vol_flag_block` 整块 resample 至 `n_cells` |
| 无模板 | 按 block1 桶线性 `+2` 回退（不推荐，易超界） |

读为 `>f8` 时，小整数对会显示为极小 denormal 浮点，这是 **整数编码**，不是物理浮点场。

### 6.4 `LS_MatOfElements` 材料 ID

- vendor FLD 材料 ID 为 **1–7**（无 0）。
- 自 SDAT 写出时，将 **0 映射为 1**（`write_fld_from_mesh` 内 `mat_export`）。
...
与 `LS_VolumeGeometryArray` 同为 **112 字节** 几何前导区；`count_val` = meta 数组长度（**ex3 为 16**，ex4 为 18），`first_block_bc` 常为 4608。
...
| block4 | 72 字节（18×i32，每槽 = meta[i]×4） |
...
3. **Relocating volumes / CreateVolFlag64_2**：检查 block2 是否全零、`lo >= n_cells`、block1 半和是否等于 `n_cells`。
4. **节布局**：`python fld_parser.py your.fld --validate-scpost` 或 `validate_scpost_geometry()` / `vol_flag_pair_issues()`。
5. **案例对比**：`python tests/compare_ex3_fld.py`（ex3 官方 vs 生成）。
...
| 功能 | 模块 |
|------|------|
| 解析 / 校验 | `fld_model.py`（`parse_fld`, `validate_scpost_geometry`, `vol_flag_pair_issues`） |
| 写出 | `fld_writer.py` |
