...
fld_primary = {p: ids[0] for p, ids in fld_at.items()}

# simulate partition split i=30
I_SPLIT = 30
...
mismatch = sum(1 for p in fld_primary if node_ids[p][0] != fld_primary[p])
print("primary mismatch", mismatch, "total nodes", next_id - 1)
for p in [(0, 0, 0), (30, 0, 0), (0, 1, 0)]:
    print(p, "built", node_ids[p], "fld", fld_at[p])
